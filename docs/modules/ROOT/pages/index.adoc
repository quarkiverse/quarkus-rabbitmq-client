...
== Migration
When updating to a new version of the extension always make sure you check out the https://github.com/quarkiverse/quarkus-rabbitmq-client/blob/main/CHANGELOG.md[changelog]
...
== Metrics
Both Micrometer and OpenTelemetry Metrics are supported. Metrics are enabled by default. You can opt-out of metrics opt-out
by specifying `quarkus.rabbitmqclient.metrics.enabled=false`. This will disable all metrics gathering.
...
To enable OpenTelemetry metrics add the `quarkus-opentelemetry` extension to the project and make sure you enable it, as
metrics is not enabled by default. To enable it set:
...
include::includes/quarkus-rabbitmq-client.adoc[leveloffset=+1, opts=optional]
